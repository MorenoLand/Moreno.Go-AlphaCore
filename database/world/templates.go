...
type CreatureTemplate struct {
	...
	VendorID, TrainerID, TrainerClass, TrainerType              int64
...
func (s *Store) CreatureTemplate(entry int64) (CreatureTemplate, bool, error) {
	...
	err := s.db.QueryRow(`SELECT entry, display_id1, name, COALESCE(subname, ''), static_flags, level_min, level_max, faction, scale, unit_class, unit_flags, npc_flags, type, beast_family, health_multiplier, mana_multiplier, armor_multiplier, damage_multiplier, damage_variance, base_attack_time, ranged_attack_time, COALESCE(vendor_id, 0), COALESCE(trainer_id, 0), COALESCE(trainer_class, 0), COALESCE(trainer_type, 0) FROM creature_template WHERE entry = ?`, entry).Scan(&creature.Entry, &creature.DisplayID1, &creature.Name, &creature.Subname, &creature.StaticFlags, &creature.LevelMin, &creature.LevelMax, &creature.Faction, &creature.Scale, &creature.UnitClass, &creature.UnitFlags, &creature.NPCFlags, &creature.Type, &creature.BeastFamily, &creature.HealthMultiplier, &creature.ManaMultiplier, &creature.ArmorMultiplier, &creature.DamageMultiplier, &creature.DamageVariance, &creature.BaseAttackTime, &creature.RangedAttackTime, &creature.VendorID, &creature.TrainerID, &creature.TrainerClass, &creature.TrainerType)
	if err == sql.ErrNoRows {
