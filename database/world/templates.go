...
func (s *Store) CreatureTemplate(entry int64) (CreatureTemplate, bool, error) {
	...
	values := []any{&creature.Entry, &creature.DisplayID1, &creature.Name, &creature.Subname, &creature.StaticFlags, &creature.LevelMin, &creature.LevelMax, &creature.Faction, &creature.Scale, &creature.UnitClass, &creature.UnitFlags, &creature.NPCFlags, &creature.Type, &creature.LootID, &creature.PickpocketLootID, &creature.SkinningLootID}
	for index := range creature.SpellIDs {
		values = append(values, &creature.SpellIDs[index])
	}
	values = append(values, &creature.BeastFamily, &creature.HealthMultiplier, &creature.ManaMultiplier, &creature.ArmorMultiplier, &creature.DamageMultiplier, &creature.DamageVariance, &creature.BaseAttackTime, &creature.RangedAttackTime, &creature.VendorID, &creature.TrainerID, &creature.TrainerClass, &creature.TrainerType, &creature.SpellListID, &creature.GoldMin, &creature.GoldMax)
	err := s.db.QueryRow(`SELECT entry, display_id1, name, COALESCE(subname, ''), static_flags, level_min, level_max, faction, scale, unit_class, unit_flags, npc_flags, type, COALESCE(loot_id, 0), pickpocket_loot_id, COALESCE(skinning_loot_id, 0), spell_id1, spell_id2, spell_id3, spell_id4, beast_family, health_multiplier, mana_multiplier, armor_multiplier, damage_multiplier, damage_variance, base_attack_time, ranged_attack_time, COALESCE(vendor_id, 0), COALESCE(trainer_id, 0), COALESCE(trainer_class, 0), COALESCE(trainer_type, 0), COALESCE(spell_list_id, 0), COALESCE(gold_min, 0), COALESCE(gold_max, 0) FROM creature_template WHERE entry = ?`, entry).Scan(values...)
	if err == sql.ErrNoRows {
