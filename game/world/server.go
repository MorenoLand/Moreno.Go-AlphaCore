...
func (s *WorldServer) handle(connection net.Conn) {
	...
	for {
		...
		case packet.CMSGTaxiQueryAvailableNodes:
			if active == nil {
				return
			}
			responses, err = s.taxiQueryNodes(active, message.Data)
		case packet.CMSGTaxiNodeStatusQuery:
			if active == nil {
				return
			}
