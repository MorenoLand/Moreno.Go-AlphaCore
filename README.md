# ![logo](.github/logo-small.png) Go server emulator for World of Warcraft 0.5.3 Alpha
...
> [!NOTE]
> The Go port is being developed in focused subsystems. The current slice provides SQLite data import, packet/SRP6 primitives, login, realm, proxy, world authentication, character create/list/delete, player-login and starting-inventory update packets, nearby creature and gameobject updates, DBC-driven player spells, item spell use, aura durations and periodic effects, item/page/quest/creature/gameobject queries, WHO, friend/ignore state, party invite/accept/disband/leader state, guild create/invite/roster/rank/chat/emblem state, channel join/list/chat/administration, vendor inventory/purchase/sale, quest-giver status/menu/details/progression/rewards, banker and tabard activation, bug tickets, played time, action and spell-button persistence, random roll, LFG, selection, target, chat/whisper, text emotes, readable item pages, basic inventory mutations, ping, time, movement persistence, and logout; broader gameplay remains in progress.
