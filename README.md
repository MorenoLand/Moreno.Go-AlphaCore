...
> [!NOTE]
> The Go port is being developed in focused subsystems. The current slice provides SQLite data import, packet/SRP6 primitives, login, realm, proxy, world authentication, character create/list/delete, player-login update packets, item/page/quest/creature/gameobject queries, WHO, friend/ignore state, played time, action and spell-button persistence, random roll, LFG, selection, target, chat/whisper, text emotes, ping, time, movement persistence, and logout; broader gameplay remains in progress.
